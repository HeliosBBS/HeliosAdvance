...
at login and on Exhausted show the busy screen and close; on any other Refused from `claim`
show the "not accepting callers" screen and close; on Unavailable from `claim` show the "not accepting
...
login calling one directly gets the same outcome as through a tool, less the gate, except
applyChanges, createBoard and resetSecret, which the database refuses to any connection not
opened with the administrator credential. The layout
...
|---|---|---|---|---|
| applyChanges | `board.upgrade`, on the connection `openWith` opened with the administrator credential | actor; the changes are the ones the setup tool's version ships | the changes applied, and the minimum engine version after | Denied, Refused (not under the administrator credential; the setup tool's version is below the board's minimum; an active server with a live lease is below a change's oldest usable version, naming those servers), Conflict (a concurrent first run), Unavailable |
| resetSecret | `login.reset`, on the connection `openWith` opened with the administrator credential | actor, server | the new secret (returned once, to be written into the record) | Denied, NotFound, Refused (not under the administrator credential; the login is not active), Unavailable |
| createBoard | `board.create`, on the connection `openWith` opened with the administrator credential | actor, board name, first server's display name, node count, HTTP connection limit (default 256), transport, address, trust-anchor fingerprint, the key-encryption key identifier | server ID, range, the first server's login name and secret (returned once, to be written into the record) | Denied, Invalid, Refused (a board exists; a change the setup tool's version ships is not applied; not under the administrator credential), Conflict (a concurrent first run), Unavailable |
...
| setNodeCount | `board.administer` | actor, server, count, expected layout version | range | Denied, Invalid, NotFound, Conflict (layout changed), Refused (nodes in use; re-plan needed; node numbers exhausted), Unavailable |
...
| claimed at | database clock, stamped by the database at each claim |
...
applied at, and an increasing identifier for order. Written only by applyChanges under the
administrator credential. Each engine version carries the identifiers of the changes it
requires.
...
2. **Renew lease** (every renewal interval in Serving, Degraded and Draining): compare-and-set
   ...
   with live leases, and the IDs of servers in `removing`. When no rows matched, read back own status all the same, and
   ...
   holding that target's server row and its Login rows, with its own operation deadline).
...
5. **Layout operations** (createBoard, addServer, setNodeCount, applyReplan, removeServer):
   the cluster mutex (createBoard on an empty database inserts the board row and holds it,
   which is the mutex; a second insert is Conflict); then, for createBoard and addServer,
   the settings state row; then, for
   createBoard, addServer and removeServer, the server row created or removed; then the
   layout rows involved in ascending server ID; then, for createBoard and addServer, the
   setting row `setWithin` writes; then the affected node rows in ascending number; then,
   for createBoard, addServer and removeServer, the Login rows created or disabled; never
   the server row of a server that keeps running. Each takes the actor and
   records its entry inside itself; createBoard and addServer compute the normalised name
   themselves. Each re-reads state after
...
6. **Drain** (a host stop, and any Fatal while a lease is held and the database is
   ...
   record `server.stop` (actor kind `engine`: a process cannot tell an operator's stop from a
   host shutdown) with its cause: `host stop` or the Fatal's reason. Zero rows on the first step means a
...
15. Under server A's login through the harness, a renewal naming server B's ID and generation, a claim's compare-and-set on a node B owns, a direct write of B's lease fields or of the occupant fields of a node B owns, a direct write of the board row, a layout row, a setting row or A's own status field, a change to the owner or number of a node A owns, and an inserted server row or node row → each rejected by the database; nothing changed.
...
30a. Under server A's login through the harness: a Login row inserted, changed or deleted directly, `createLogin` naming A itself or active server B (each already bound to an active Login row), `disableLogin` for active B, `revokeLogin` for active B and for a server in `removing` (no server login may call these three directly), `completeRemoval` for active B, `resetSecret` for B and for A itself, createBoard presenting the stored key identifier, address and transport while the first server has never acquired (the recovery input), and applyChanges → each rejected by the database with no `board.recover` or `board.upgrade` entry; B keeps serving through its next renewal and A's login still authenticates; removeServer(B) called directly under A's login → succeeds, and `server.remove` names A as origin server. After renewals on every server, the administrator credential still authenticates and `listLogins` reports only the logins createLogin made.
...
34. Each gated registry operation, applyChanges, resetSecret, listServers, previewReplan and whosOnline.list included, with access control forced to error → Denied; nothing changed; no audit entry.
35. Each gated registry operation by an actor without the permission → Denied.
