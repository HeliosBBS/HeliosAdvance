...
- **A login** is what a program authenticates to the database with. Disabling a login takes
  part in the transaction that requests it and refuses new connections from commit; ending
  its open connections and revoking it happen after commit and are retried until done. A
  login may create logins holding only its own rights or fewer. Audit entries are owned by a
  role no program's login holds, so no login any program holds or creates can change or
  delete one.
...
that the stack provides, so every negative test in the subsystem documents can force its
dependency to fail.
