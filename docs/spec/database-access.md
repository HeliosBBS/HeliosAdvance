...
| key-encryption key | the board's key for values sealed at rest: 256 bits from the host's cryptographic random source, generated by the setup tool at first run and copied to each joining server through join v1; sensitive |
| record version | increased by every `writeRecord` |
...
key-encryption key; the sealed form is the key identifier (a 128-bit fingerprint of the key),
a 96-bit nonce that is never reused under one key, the ciphertext, and the tag. `unseal`
refuses any form whose key identifier is not this key's or whose tag does not verify. The key
