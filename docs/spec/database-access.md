...
| openWith | the setup tool only, at first run: address, transport, trust anchor, and a credential that is never stored or logged | a connection under that credential, subject to the same transport decision | Fatal (as `open`), Invalid (transport invalid for the address), Unavailable |
...
First run, in order: the setup tool generates the key; writes the record with transport,
address, trust anchor and key (server ID and login empty); opens a connection with
`openWith` and the administrator credential; calls cluster's createBoard; writes the server
ID, login and secret into the record; discards the administrator credential.
